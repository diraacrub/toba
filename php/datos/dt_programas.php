...
class dt_programas extends catedras_datos_tabla
{
	...
	function get_listado($filtro=array())
	{
		...
		FROM
			programas as t_p
		WHERE
			t_p.observaciones IS NULL
			OR t_p.observaciones NOT ILIKE '%[ocultar]%'
		...
		function get_listado_repuesto2026($filtro=array())
	{
		$where = array();
		if (isset($filtro['id_programa'])) {
			$where[] = "id_programa = ".quote($filtro['id_programa']);
	...
	function get_datos_programa($id_programa)
{
	...
	JOIN
		materias as t_m ON t_p.id_materia_prog = t_m.id_materia
	WHERE
		(t_p.observaciones IS NULL OR t_p.observaciones NOT ILIKE '%[ocultar]%')                
		AND
...
function get_listado_datos_docentes($filtro=array())
	{
		$where = array();
		if (isset($filtro['id_programa'])) {
			$where[] = "id_programa = ".quote($filtro['id_programa']);
...
LEFT JOIN materias AS m
	ON m.id_materia = t_p.id_materia_prog
LEFT JOIN desarrollo.apex_usuario AS u
	ON u.usuario = t_p.legajo_resp::text            
ORDER BY
	t_p.apellido_resp
";
