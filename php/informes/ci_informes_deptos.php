...
class ci_informes_deptos extends ci_base_operaciones
{
	...
	$perfil_usuario
		...
		if (array_intersect($perfiles_funcionales, $excepciones)) {
			...
			$datos = $this->dep('datos')->tabla('informes')->get_listado_filtrado_depto($usuario_id, $perfil_usuario);
		}
	...
	$perfil_usuario
		...
		if (array_intersect($perfiles_funcionales, $excepciones)) {        
			...
			$datos = $this->dep('datos')->tabla('informes')->get_listado_enviados_depto($usuario_id, $perfil_usuario);
		}
	...
	if (isset($datos['estado_informe'])) {
		// Obtener el estado actual
		$estado_nuevo = $datos['estado_informe'];

		if ($estado_nuevo === 'docente') {
			// Si el estado cambia a 'docente', eliminar el contenido de 'firma_doc'
			$datos['firma_doc_inf'] = '';
		} elseif ($estado_nuevo === 'aprobado') {
			// Si el estado cambia a 'aprobado', actualizar 'firma_dto' con la firma electrónica
			...
			$datos['firma_dto_inf'] = "Firmado electrónicamente por $nombre_completo - Responsable de Departamento/Delegación/Coordinación de Carrera - $formatted_timestamp";
		}
	}
