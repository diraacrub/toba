<?php
class ci_sac_control_programas extends catedras_ci
{
...
function conf__cuadro(toba_ei_cuadro $cuadro)
{
	...
	if (in_array('saccrub', $perfil_usuario)) {
		...
		$deptos_principales = array('ECOLOGÍA', 'MATEMÁTICA', 'FÍSICA');
	}
	// Si el array de perfiles funcionales contiene 'sacfale'
	elseif (in_array('sacfale', $perfil_usuario)) {
		// Para 'sacfale' la lista es otra:
		$deptos_principales = array('EXPLOTACIÓN DE RECURSOS ACUÁTICOS', 'ECOLOGÍA');
	}
	...
	if (in_array($usuario_id, $excepciones)) {
		$datos = $this->dep('datos')->tabla('programas')->get_listado_estado_docente_depto();
	} else {
		// Pasamos la lista de deptos_principales a la consulta filtrada
		$datos = $this->dep('datos')->tabla('programas')->get_listado_filtrado_control_sac($usuario_id, $deptos_principales);
	}
